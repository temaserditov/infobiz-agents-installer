...
{
  "version": "$VERSION",
  "sourceMode": "$AGENT_SOURCE_MODE",
  "hermesRequires": ">=0.18.2",
  "distributionOwned": ["default", "marketer", "copywriter", "designer", "tech", "webshell-docs"],
  "profiles": ["default", "marketer", "copywriter", "designer", "tech"],
  "generatedAt": "$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
}
JSON

COPYFILE_DISABLE=1 tar --uid 0 --gid 0 -C "$BUILD_DIR" -czf "$TARBALL" profile
/bin/rm -rf "$BUILD_DIR"
echo "$TARBALL"
